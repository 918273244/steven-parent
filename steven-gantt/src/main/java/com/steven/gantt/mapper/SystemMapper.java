...
public interface SystemMapper {
    ...
    //新增假期
    @Insert("insert into HOLIDAYINFO(DATETIME,CREATETIME, id) values(#{dateTime},sysdate, SEQ_HOLIDAYINFO.NEXTVAL) ")
    ...
    @Select("SELECT * FROM   (  SELECT A.ID,A.RMTNAME, ROWNUM RN FROM ( SELECT * FROM RMTSYS WHERE DELETESTATUS = 0 order by id desc ) A   WHERE ROWNUM <= #{endIndex}  )  WHERE RN > #{startIndex} ")
    ...
    @Insert("insert into RMTSYS(ID,CREATETIME,RMTNAME, UPDATETIME) values(SEQ_RMTSYS.NEXTVAL, sysdate , #{rmtName}, sysdate)")
    int addRmt(@Param("rmtName") String name);

    @Insert("update RMTSYS set DELETESTATUS = 1, UPDATETIME = sysdate where id = #{id}")
